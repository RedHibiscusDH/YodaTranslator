...
def drawBlocks(img, blocks):    #отрисовка блок распознаного текста от easyocr
    for i in blocks:
        ax = int(i[0][0][0])
        ay = int(i[0][0][1])
        bx = int(i[0][2][0])
...
def drawParagraphs(img, paragr):    #отрисовка блок распознаного текста от easyocr
    for i in paragr:
        ax = int(i[0][0][0])
        ay = int(i[0][0][1])
        bx = int(i[0][1][0])
...
def recognizeBlockOfText(start_file):      #распознавание текста
    reader = easyocr.Reader(["en"], gpu = True)
    return reader.readtext(start_file, paragraph = False)

#-------------------------

def recognizeParagraphes(blocks):
...
def recognizeText(img_file, drBlocks = False, drParagraphs=False):
    blc = recognizeBlockOfText(img_file)
    prg = recognizeParagraphes(blc)
    if drBlocks:
        img = cv2.imread(img_file)
        img = drawBlocks(img, blc)
        cv2.imwrite(img_file, img)
    if drParagraphs:
        img = cv2.imread(img_file)
        img = drawParagraphs(img, prg)
        cv2.imwrite(img_file, img)
    return prg

#-------------------------

def recognizeTextTesseract(img_file, paragrapth):
